...
class ESBTPMatiereSeeder extends Seeder
{
    ...
    foreach ($matieresByFiliere as $classeCode => $matieres) {
        $classe = ESBTPClasse::where('code', $classeCode)->first();

        if (!$classe) {
            $this->command->warn("Classe non trouvée : $classeCode");
            continue;
        }

        foreach ($matieres as $matiereData) {
            $matiere = ESBTPMatiere::updateOrCreate(
            ...
            $matiere->classes()->syncWithoutDetaching([$classe->id]);
        }
    }
}
