...
class ESBTPBulletinController extends Controller
{
    ...
    public function genererPDFParParams(Request $request)
    {
        try {
            ...
            // Vérifier si des moyennes dans esbtp_resultats sont nulles
            ...
            // Vérifier la configuration des matières
            if ($bulletin && !$bulletin->config_matieres) {
                return back()->with('error', 'La configuration des matières n\'a pas été effectuée. Veuillez d\'abord configurer les matières.');
            }

            // Vérifier l'existence des professeurs
            if ($bulletin && !$bulletin->professeurs) {
                return back()->with('error', 'Les professeurs n\'ont pas été assignés. Veuillez d\'abord assigner les professeurs.');
            }
